...
class Piece {
  ...
  Offset centerDragAnchorStrategy(Draggable<Object> d, BuildContext context, Offset position) {
    StdCoords c = getCenter();
    return Offset(50 * c.getXCoords().toDouble() + 25, 50 * c.getYCoords().toDouble() + 25);
  }
  ...
  void paint(Canvas canvas, Size size) {
    if (_offsets == []) {
      ...
      Paint paint = Paint()
        ...
        ..strokeWidth = 0;
      Path path = Path();
      double x = _off.getOffset().dx;
      double y = _off.getOffset().dy;
